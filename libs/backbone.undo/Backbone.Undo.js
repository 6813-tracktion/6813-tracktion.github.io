 ...
 */
/*
 * Changes for Tracktion:
 * - Explicit change fusion to support our drag interactions.
 * - Trigger an event when undo/redo availability changes.
 */
(function (_, Backbone, undefined) {
	...
	function managerUndoRedo (which, manager, stack, magic) {
		...
		manager.trigger("availabilityMayHaveChanged", manager);
	}
	...
	function addToStack(stack, type, args, undoTypes) {
		if (stack.track && !stack.isCurrentlyUndoRedoing && type in undoTypes &&
			...
			if (hasKeys(res, "object", "before", "after")) {
				res.type = type;
				// Explicit fusion. ~ Matt
				res.magicFusionIndex = stack.fusionIndex;
	...
	var UndoTypes = {
		...
		"commit": {
			"undo": function (manager, before, after) {},
			"redo": function (manager, before, after) {},
			"on": function (manager) {
				return {
					object: manager,
					before: null,
	...
	UndoManager = Backbone.Model.extend({
		...
		commit: function () {
			// Add a dummy entry so empty "transactions" take a space in the fused undo stack.
			// Currently needed for consistency with nonempty transactions that have no net effect in Tracktion,
			// such as dragging something back and forth.
			// XXX: Would we prefer to have all transactions with no net effect not take a space?
			this._addToStack("commit");
			this.stack.fusionIndex++;
		},
		rollback: function () {
