      ...
      <% _.each(reversedDays(), function(d, i) { %>
        ...
        <g transform="translate(100, <%= DAY_HEIGHT_PX * (i + 1/2) %>)">
          <% if(urlVar('cumulative') != 'line'){ %>
            <rect class="cumulative" y="<%= -DAY_HEIGHT_PX/2 - (i == 0 ? 3 : 0) %>" height="<%= DAY_HEIGHT_PX + (i == 0 ? 3 : 0) %>" width="<%= rm2p(cumulative(d)) %>" />
            <rect class="cumulative-curr" y="<%= -DAY_HEIGHT_PX/2 - (i == 0 ? 3 : 0) %>" height="<%= DAY_HEIGHT_PX + (i == 0 ? 3 : 0) %>" width="<%= rm2p(cumulative(d)) - rm2p(cumulative(d-1) || 0) %>" x="<%= rm2p(cumulative(d-1) || 0) %>" />
            <line class="cumulative end" y1="<%= -DAY_HEIGHT_PX/2 - (i == 0 ? 3 : 0) %>" y2="<%= DAY_HEIGHT_PX/2 %>" x1="<%= rm2p(cumulative(d)) %>" x2="<%= rm2p(cumulative(d)) %>" />
            <% if(cumulative(d) > cumulative(d - 1)) { %>
              ...
              <!-- inner lines -->
              <line class="cumulative end" y1="<%= -DAY_HEIGHT_PX/2 - (i == 0 ? 3 : 0) %>" y2="<%= DAY_HEIGHT_PX/2 %>" x1="<%= rm2p(cumulative(d-1)) %>" x2="<%= rm2p(cumulative(d-1)) %>" />
              <line class="cumulative end" y1="<%= -DAY_HEIGHT_PX/2 - (i == 0 ? 3 : 0) %>" y2="<%= -DAY_HEIGHT_PX/2 - (i == 0 ? 3 : 0) %>" x1="<%= rm2p(cumulative(d-1)) %>" x2="<%= rm2p(cumulative(d)) %>" />
            <% } %>
          <% } else { %>
            <% if(i > 0) { %>
